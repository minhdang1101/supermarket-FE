...
export default function CheckoutPage() {
  ...
  const handleCompleteCheckout = async () => {
    ...
    if (paymentMethod === 'VNPAY') {
      vnpayWindow = window.open('about:blank', '_blank');
      if (vnpayWindow) {
        vnpayWindow.opener = null;
      }
    }
    ...
    try {
      ...
      // If VNPay selected, open the payment gateway in a new tab.
      if (paymentMethod === 'VNPAY') {
        const vnpayRequest = {
        ...
        if (vnpayResponse.code === '00' && vnpayResponse.paymentUrl) {
          ...
          if (vnpayWindow) {
            vnpayWindow.location.href = vnpayResponse.paymentUrl;
          } else {
            window.open(vnpayResponse.paymentUrl, '_blank', 'noopener,noreferrer');
          }
          setIsPaymentDialogOpen(false);
          toast.info('Đã mở cổng thanh toán VNPay trong tab mới');
          return;
        } else {
          vnpayWindow?.close();
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-100">
      ...
      <div className="flex-1 min-h-0 flex gap-4 p-4 overflow-hidden">
        ...
        <div className="flex-1 min-h-0 flex flex-col min-w-0 bg-white rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="w-full lg:w-[420px] min-h-0 flex flex-col bg-white rounded-xl shadow-sm overflow-hidden">
          {/* Cart Header */}
          <div className="p-4 bg-slate-50 border-b flex shrink-0 items-center justify-between">
          ...
          {/* Totals */}
          <div className="p-4 border-t bg-slate-50 space-y-2 text-sm shrink-0">
